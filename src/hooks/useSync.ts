...
export function useSync(
  ...
  const currentStepRef = useRef<SyncStep | null>(currentStep);
  currentStepRef.current = currentStep;
  const currentSubStepRef = useRef<string | null>(currentSubStep);
  currentSubStepRef.current = currentSubStep;
  const targetClRef = useRef<string>(targetCl);
  targetClRef.current = targetCl;
    ...
    channel.onmessage = (event: SyncEvent) => {
      switch (event.event) {
        case "stepStarted":
          ...
          setCurrentSubStep(event.data.subStep ?? null);
        ...
        case "syncCancelled": {
          ...
          syncStateRef.current = "idle";
          setIsCancelling(false);
          // Pitfall 5: snapshot BEFORE setTargetCl("") so a Target-CL cancel
          // still looks up steps.p4Sync.toCl with {cl}.
          const step = event.data.step || currentStepRef.current || "";
          const sub = currentSubStepRef.current;
          const clSnap = targetClRef.current;
          setTargetCl("");
          setCurrentSubStep(null);
          setLastSyncResult({
            status: "cancelled",
