 ...
 *
 * Two exports:
 *   - classifyProxyError(errorLike): pure error → machine kind.
 ...
 *       but GitHub never came back). Does not author display strings.
 *   - runProxyConnectionTest({ proxyUrl, timeout }): drives a single
 *       `check({ proxy, timeout })` call and maps the outcome to a
 *       ProxyTestResult (null → ok, Update → ok+hasNote, throw → classified).
 *
...
import { proxyUrlHasCredentials } from "./updaterProxyPrivacy";
 ...
 *   - { kind: "ok"; hasNote: true }     when check() resolves a truthy Update —
...
export async function runProxyConnectionTest(opts: {
  ...
  if (proxyUrlHasCredentials(opts.proxyUrl)) return { kind: "credentials" };
  try {
