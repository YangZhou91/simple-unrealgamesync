...
import { proxyUrlHasCredentials, PROXY_CREDENTIALS_ERROR } from "./updaterProxyPrivacy";
...
function getStore(): Promise<Store> {
  if (!storePromise) {
    // Explicit flushing also covers migration of legacy credential URLs.
 ...
 * back to {@link DEFAULT_UPDATER_PROXY_URL}. Legacy credential-bearing URLs
 * are cleared and proxy routing disabled before returning any settings.
 */
export async function loadUpdaterSettings(): Promise<UpdaterSettings> {
  ...
  if (typeof urlRaw === "string" && proxyUrlHasCredentials(urlRaw)) {
    await store.set(KEY_URL, DEFAULT_UPDATER_PROXY_URL);
    await store.set(KEY_ENABLED, false);
    await store.save();
    return { proxyEnabled: false, proxyUrl: DEFAULT_UPDATER_PROXY_URL };
  }
...
export async function saveUpdaterSettings(s: UpdaterSettings): Promise<void> {
  if (proxyUrlHasCredentials(s.proxyUrl)) {
    throw new Error(PROXY_CREDENTIALS_ERROR);
  }
  const store = await getStore();
  const proxyUrl =
