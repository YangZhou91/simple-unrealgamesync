 ...
 *     maps null → ok, Update → ok+hasNote, throw → classified
 *
...
describe("runProxyConnectionTest", () => {
  it("rejects credentials without sending them to the updater", async () => {
    const { runProxyConnectionTest } = await loadFreshModule();
    expect(await runProxyConnectionTest({ proxyUrl: ["http://alice:secret", "localhost:7897"].join("@"), timeout: 8000 })).toEqual({ kind: "credentials" });
  ...
  it("resolves to { kind: 'ok', hasNote: true } when check resolves an Update", async () => {
    currentCheck = async () => ({ version: "1.5.0" });
    const m = await loadFreshModule();

    const r = await m.runProxyConnectionTest({ proxyUrl: "http://localhost:7897", timeout: 8000 });
    expect(r).toEqual({ kind: "ok", hasNote: true });
  });

  it("resolves to { kind: 'ok' } with hasNote absent or falsy when check resolves null", async () => {
    currentCheck = async () => null;
    const m = await loadFreshModule();
