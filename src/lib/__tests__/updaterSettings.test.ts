...
describe("updaterSettings", () => {
  it.each([["http://alice:secret", "localhost:7897"].join("@"), ["socks5://token", "localhost:1080"].join("@"), "broken:secret@proxy"])("rejects credential URL %s before storage", async (proxyUrl) => {
    const fake = makeFakeStore({});
    currentStore = fake;
    const { saveUpdaterSettings } = await loadFreshModule();
    await expect(saveUpdaterSettings({ proxyEnabled: true, proxyUrl })).rejects.toThrow("proxy_credentials_not_supported");
    expect(fake.entries).toEqual({});
    expect(fake.getSaveCalls()).toBe(0);
  });

  it("clears legacy credentials and disables the proxy before returning settings", async () => {
    const fake = makeFakeStore({ "updater.proxy_enabled": true, "updater.proxy_url": ["http://alice:secret", "localhost:7897"].join("@") });
    currentStore = fake;
    const { loadUpdaterSettings, DEFAULT_UPDATER_PROXY_URL } = await loadFreshModule();
    expect(await loadUpdaterSettings()).toEqual({ proxyEnabled: false, proxyUrl: DEFAULT_UPDATER_PROXY_URL });
    expect(fake.entries["updater.proxy_url"]).toBe(DEFAULT_UPDATER_PROXY_URL);
    expect(fake.entries["updater.proxy_enabled"]).toBe(false);
