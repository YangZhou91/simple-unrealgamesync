 ...
 * - `path`: raw depot/local path for local diagnosis. Empty-string sentinel for
...
export type SyncEvent =
  | { event: "stepStarted"; data: { step: string; description: string; subStep?: string | null } }
  | { event: "stepCompleted"; data: { step: string; success: boolean } }
  | { event: "progress"; data: { current: number; total: number; currentFile: string; bytesDone?: number | null; bytesTotal?: number | null; bytesRate?: number | null; percent?: number | null; phase?: string | null } }
...
export type LastSyncResult = {
  ...
  fileCount: number;
  epochMs: number;
  /** Cancelled only — machine step token. */
  step?: string;
  /** Cancelled only — snapshotted sub-step token. */
  subStep?: string;
};
...
export interface GitBranchInfo {
  ...
  behind: number;
  remote: string;
  // quick-260724-tgj: snake_case to match the backend's default serde
  // (GitStatusInfo has NO rename_all — first multi-word field confirms casing).
  short_hash: string;
  is_detached: boolean;
}
