...
impl P4Executor {
    ...
    /// Task 3 human-verify checkpoint against the real ExampleGame workspace.
...
// ========================================================================
//
// Surfaces ALL files with abnormal p4 status in the ExampleGame structural
...
/// (e.g. `D:\ExampleDepot\...`). Used by `extract_warn_path` to recognize local
...
/// `//depot/ExampleGame/Config/DefaultEngine.ini#1 - added as D:\\...`). The
...
fn extract_whitelist_relative_path(path: &str) -> String {
    ...
    // the segment onward (e.g. "...ExampleGame/Config/DefaultEngine.ini" -> "Config/DefaultEngine.ini").
    ...
    // (e.g. "//ExampleDepot/ExampleGame/ExampleGame.uproject" -> "ExampleGame/ExampleGame.uproject").
...
pub fn build_audit_whitelist_args(root_path: &str, project_dir: &str) -> Vec<String> {
    ...
    // Build the relative project prefix (e.g. "ExampleGame" or "UnrealEngine/ExampleGame").
    let project_rel = project_path
        .strip_prefix(root)
        .unwrap_or(Path::new(project_dir))
        .to_string_lossy()
...
";
    ...
    fn test_parse_reconcile_line_added_as() {
        ...
        let r = parse_reconcile_line(
            "//depot/ExampleGame/Config/DefaultEngine.ini#1 - added as D:\\ExampleDepot\\ExampleGame\\Config\\DefaultEngine.ini",
        );
    ...
    fn test_parse_reconcile_line_deleted_as() {
        ...
        let r = parse_reconcile_line(
            "//depot/ExampleGame/Source/Foo.cpp#3 - deleted as D:\\ExampleDepot\\ExampleGame\\Source\\Foo.cpp",
        );
    ...
    fn test_parse_reconcile_line_edited_also_opened() {
        ...
        let r = parse_reconcile_line("//depot/ExampleGame/Source/Bar.h#2 - edited, also opened");
    ...
    fn test_parse_reconcile_line_strips_project_prefix() {
        ...
        let r = parse_reconcile_line(
            "ExampleGame/Config/DefaultEngine.ini#1 - added as D:\\ExampleDepot\\ExampleGame\\Config\\DefaultEngine.ini",
        );
        assert_eq!(r, Some((WorkspaceHealthCategory::NotInDepot, "Config/DefaultEngine.ini".to_string())));
    }
    ...
    fn test_parse_where_line_mapped_triple() {
        ...
        let r = parse_where_line(
            "//ExampleDepot/ExampleGame/Config/DefaultEngine.ini //client/ExampleGame/Config/DefaultEngine.ini D:\\ExampleDepot\\ExampleGame\\Config\\DefaultEngine.ini",
        );
    ...
    fn test_parse_where_line_unmapped() {
        ...
        let r = parse_where_line("- //ExampleDepot/ExampleGame/ExampleGame.uproject - file(s) not in client view.");
        assert_eq!(r, Some((false, "ExampleGame/ExampleGame.uproject".to_string())));
    }
    ...
    fn test_parse_resolve_line_merging() {
        ...
        let r = parse_resolve_line(
            "info: //ExampleDepot/ExampleGame/Config/DefaultEngine.ini#1 - merging //ExampleDepot/ExampleGame/Config/DefaultEngine.ini#1,#2",
        );
        assert_eq!(
            r,
            Some((
    ...
    fn test_parse_resolve_line_must_resolve() {
        ...
        let r = parse_resolve_line(
            "//ExampleDepot/ExampleGame/Source/Foo.cpp#3 - must resolve //ExampleDepot/ExampleGame/Source/Foo.cpp#3,#4",
        );
        assert_eq!(
            r,
            Some((
    ...
    fn test_parse_resolve_line_pathless_leading_dash_is_none() {
        ...
        let r = parse_resolve_line("info: - must resolve //ExampleDepot/ExampleGame/Source/Foo.cpp#3,#4");
        assert_eq!(r, None);
    }
    ...
    fn test_parse_resolve_line_per_path_no_file_warning_is_none() {
        ...
        // client Example_Stream_Developer that returned zero resolves: the
        // panel had shown these 3 whitelist globs as phantom NeedsResolve
        // entries (resolve=3) before the Warning-skip fix.
        for line in [
            "warning: UnrealEngine/ExampleGame/Config/... - no file(s) to resolve.",
            "warning: UnrealEngine/ExampleGame/Source/... - no file(s) to resolve.",
            "warning: UnrealEngine/ExampleGame/ExampleGame.uproject - no file(s) to resolve.",
        ] {
    ...
    fn test_parse_resolve_line_uproject_relative() {
        ...
        let r = parse_resolve_line(
            "//ExampleDepot/ExampleGame/ExampleGame.uproject#2 - merging //ExampleDepot/ExampleGame/ExampleGame.uproject#2,#3",
        );
        assert_eq!(
            r,
            Some((
                WorkspaceHealthCategory::NeedsResolve,
                "ExampleGame/ExampleGame.uproject".to_string()
            ))
        );
    }
    ...
    fn test_build_audit_whitelist_args_resolves_unrealengine_prefix() {
        ...
        // ExampleGame layout), the whitelist entries carry the UnrealEngine/ prefix.
        ...
        let project_dir = tmp_dir.join("UnrealEngine").join("ExampleGame");
...
";
    ...
    fn test_parse_reconcile_line_info_prefixed() {
        let r = parse_reconcile_line(
            "info: //depot/ExampleGame/Config/DefaultEngine.ini#1 - added as D:\\ExampleDepot\\ExampleGame\\Config\\DefaultEngine.ini",
        );
        assert_eq!(
            r,
            Some((
    ...
    fn test_parse_reconcile_line_error_is_none() {
        ...
        assert_eq!(
            parse_reconcile_line("error: //depot/ExampleGame/Config/X.ini#1 - added as D:\\X"),
            None
        );
    }
    ...
    fn test_parse_where_line_info_prefixed_mapped() {
        let r = parse_where_line(
            "info: //ExampleDepot/ExampleGame/Config/DefaultEngine.ini //client/ExampleGame/Config/DefaultEngine.ini D:\\ExampleDepot\\ExampleGame\\Config\\DefaultEngine.ini",
        );
        assert_eq!(r, Some((true, "Config/DefaultEngine.ini".to_string())));
    }
    ...
    fn test_parse_where_line_error_prefixed_unmapped() {
        ...
        let r = parse_where_line(
            "error: - //ExampleDepot/ExampleGame/ExampleGame.uproject - file(s) not in client view.",
        );
        assert_eq!(r, Some((false, "ExampleGame/ExampleGame.uproject".to_string())));
    }
    ...
    fn test_parse_resolve_line_error_is_none() {
        ...
        assert_eq!(
            parse_resolve_line(
                "error: //ExampleDepot/ExampleGame/Config/X.ini#1 - merging //ExampleDepot/ExampleGame/Config/X.ini#1,#2"
            ),
            None
        );
    }
    ...
    fn test_parse_client_stream_info_prefixed() {
        assert_eq!(
            parse_client_stream(
                "info: Client: developer\ninfo: Stream: //Example_Depot/main\nexit: 0"
            ),
            Some("//Example_Depot/main".to_string())
        );
    }
    ...
    fn test_parse_client_stream_classic_no_stream_line_is_none() {
        assert_eq!(
            parse_client_stream("info: Client: developer\ninfo: Root: D:\\ExampleDepot\nexit: 0"),
            None
        );
    }
    ...
    fn test_resolve_non_excluded_paths_skips_project_catchall() {
        ...
        // overlap. project_candidates resolves to `UnrealEngine/ExampleGame`.
        let project_dir = ue_dir.join("ExampleGame");
        let _ = fs::remove_dir_all(&tmp_dir);
        // Project's own children — must be enumerated granularly by the first
        // loop as `UnrealEngine/ExampleGame/<child>/...`.
        ...
        assert!(
            paths.iter().any(|p| p == "UnrealEngine/ExampleGame/Content/..."),
            "granular project child Content must be present; got {:?}",
            paths
        );
        assert!(
            paths.iter().any(|p| p == "UnrealEngine/ExampleGame/Build/..."),
            "granular project child Build must be present; got {:?}",
            paths
        );

        // (B) CORE: the project-dir catch-all is ABSENT — exact-match (NOT
        // .contains("ExampleGame"), which would match the legitimate granular
        // children like `UnrealEngine/ExampleGame/Content/...`). This is the
        // phantom-bytes line that contributed ~959.7 GB to the -N denominator.
        assert!(
            !paths.iter().any(|p| p == "UnrealEngine/ExampleGame/..."),
    ...
    fn test_build_p4_sync_args_with_cl_no_engine() {
        ...
        let args = build_p4_sync_args(&options, tmp_dir.to_str().unwrap(), "ExampleGame", &options.target_cl);
        ...
        // expressed (`//...@12345` vs a granular `ExampleGame/...@12345`) is an
    ...
    fn test_warning_collector_warn_tagged() {
        ...
        c.ingest(
            "warning: //Example_Depot/.../X.uasset - no such file(s).",
            DrainOrigin::Stdout,
        );
        let out = c.finalize();
        assert_eq!(out.len(), 1);
        ...
        assert_eq!(out[0].path, "//Example_Depot/.../X.uasset");
        assert_eq!(out[0].message, "//Example_Depot/.../X.uasset - no such file(s).");
    }
    ...
    fn test_extract_warn_path_local_drive_root() {
        ...
        assert_eq!(
            extract_warn_path("D:\\ExampleDepot\\FY\\Content\\X.uasset#7 - no such file(s)."),
            "D:\\ExampleDepot\\FY\\Content\\X.uasset"
        );
    }
