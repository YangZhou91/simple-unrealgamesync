...
//!   extra `<USER>` row. This also masks usernames embedded in workspace
//!   names outside absolute paths.
...
fn patterns() -> &'static [(Regex, &'static str)] {
    PATTERNS.get_or_init(|| {
        vec![
            // Mask whole absolute paths on any drive, including slash variants.
            // Unquoted paths may contain spaces: conservatively consume through
            // the line/quote boundary rather than leak a private path tail.
            (Regex::new(r#"(?i)\b[a-z]:[\\/][^\r\n"<>|]*"#).unwrap(), "<PATH>"),
            ...
            // Bare P4 endpoints and private IPv4 addresses in connect errors.
            (Regex::new(r"\b[A-Za-z0-9._-]+:1666\b").unwrap(), "<P4PORT>"),
            (Regex::new(r"\b(?:10\.\d{1,3}\.\d{1,3}\.\d{1,3}|192\.168\.\d{1,3}\.\d{1,3}|172\.(?:1[6-9]|2\d|3[01])\.\d{1,3}\.\d{1,3})(?::\d+)?\b").unwrap(), "<HOST>"),
            ...
            (Regex::new(r"(?i)([a-z][a-z0-9+.-]*://)[^\s/]+@").unwrap(), "${1}<CRED>@"),
            // git@ ssh-style creds (D-02). Same ordering reason as above.
            (Regex::new(r"\bgit@[A-Za-z0-9._-]+:").unwrap(), r"<CRED>"),
            // UNC paths: both server and share/file names may identify projects.
            (Regex::new(r#"\\\\[^\r\n"<>|]*"#).unwrap(), "<PATH>"),
...
/// AND non-empty, compiles `r"(?i)<escaped_username>"` (using
/// `regex::escape` so metacharacters in usernames don't corrupt the pattern)
/// and returns `Some((regex, "<USER>"))`. Substring matching also catches
/// usernames embedded in workspace names. If unset/empty, returns `None`
    ...
    USERNAME_PATTERN
        .get_or_init(|| {
            ...
            std::env::var("USERNAME")
                ...
                .filter(|u| !u.is_empty())
                .and_then(|username| {
                    let pattern = format!(r"(?i){}", regex::escape(&username));
...
/// static patterns (e.g. `E:\workspaces\<you>\ExampleGame\`).
...
pub const FIXTURE_P4_CLIENT: &str = "alice-laptop-examplegame";
...
pub fn test_workspace_fixture() -> WorkspaceConfig {
    WorkspaceConfig {
        id: "ws-test".into(),
        name: "Alice's ExampleGame".into(),
        root_path: r"C:\Users\alice\workspaces\ExampleGame".into(),
...
pub const PATH_VARIANTS: &[&str] = &[
    ...
    r"C:\Users\alice\workspaces\ExampleGame\Content\Maps\Foo.uasset",
    // Non-Users drive (the D-03 fail-open case the %USERNAME% row mitigates).
    r"E:\workspaces\alice\ExampleGame\Content\Maps\Foo.uasset",
    // Another non-Users drive.
    r"D:\dev\alice\ExampleGame\Source\MyGame\MyGame.build.cs",
    // UE personal-content folder (Developers/<segment>/).
    r"ExampleGame/Content/Developers/alice/Maps/Test.umap",
    // Depot form with a Developers segment.
    r"//ExampleGame/Content/Developers/alice/Maps/Test.umap",
    // Depot form with no username (whole-masked, must not regress).
    r"//ExampleGame/Main/Content/Maps/Foo.uasset",
];
...
mod tests {
    ...
    #[test]
    fn arbitrary_private_paths_and_url_credentials_are_masked() {
        for path in [r"d:\Studio Work\Stream_alice\Secret.uasset", "e:/Studio/Stream_alice/Secret.uasset", "//ExampleDepot/Art Stream_alice/Secret.uasset", r"\\server\Private Share\Secret.uasset"] {
            let masked = redact(path);
            assert!(!masked.contains("Secret"));
            assert!(!masked.contains("alice"));
        }
        assert_eq!(redact(concat!("http://alice:password", "@", "localhost:7897")), "http://<CRED>@localhost:7897");
        assert_eq!(redact(concat!("socks5://token", "@", "localhost:1080")), "socks5://<CRED>@localhost:1080");
        assert_eq!(redact("https://github.com/example/repo"), "https://github.com/example/repo");
        assert_eq!(redact("Connect to server failed; check studio-p4:1666"), "Connect to server failed; check <P4PORT>");
        assert_eq!(redact("TCP connect to 10.20.30.40:1999 failed"), "TCP connect to <HOST> failed");
        let username = Regex::new(r"(?i)alice").unwrap();
        assert_eq!(redact_with_username_row("Stream_alice", Some((&username, "<USER>"))), "Stream_<USER>");
    }

    // ---- Filesystem paths: whole-path masking ----

    #[test]
    fn redact_masks_users_home_path_to_path_token() {
        // Project-specific tails are private too.
        assert_eq!(
            redact(r"C:\Users\alice\workspaces\ExampleGame"),
            "<PATH>"
        );
    }
    ...
    fn redact_masks_users_home_on_non_c_drive() {
        ...
        assert_eq!(
            redact(r"D:\Users\bob\workspaces\ExampleGame"),
            "<PATH>"
        );
    }
    ...
    fn redact_preserves_developers_segment_mask() {
        ...
        assert_eq!(
            redact("ExampleGame/Content/Developers/alice/Maps/Foo.umap"),
            "ExampleGame/Content/Developers/<DEVELOPER>/Maps/Foo.umap"
        );
    }
    ...
    fn redact_masks_depot_path_to_depot_token() {
        assert_eq!(
            redact("//ExampleGame/Main/Content/Maps/Foo.uasset"),
            "<DEPOT>"
        );
    }
    ...
    fn redact_strips_git_url_creds_keeps_url() {
        ...
        assert_eq!(
            redact(concat!("https://alice:ghp_token", "@", "github.com/EpicGames/UnrealEngine.git")),
    ...
    fn redact_masks_unc_host_to_host_token() {
        // Server and share/project names are all masked.
        assert_eq!(
            redact(r"\\buildfarm\share\ExampleGame"),
            "<PATH>"
        );
    }
    ...
    fn redact_path_variants_do_not_leak_username() {
        // The static catalog and optional username row cooperate:
        ...
        // applies the username pattern (escaped, case-insensitive substring).
        let user_re = Regex::new(&format!(r"(?i){}", regex::escape(FIXTURE_USERNAME)))
    ...
    #[test]
    fn redact_non_users_drive_caught_without_username_row() {
        // Absolute paths are protected even without the current OS username.
        let user_re = Regex::new(&format!(r"(?i){}", regex::escape(FIXTURE_USERNAME)))
            .expect("fixture username must compile");
        let variant = r"E:\workspaces\alice\ExampleGame\Content\Maps\Foo.uasset";
        // The static catalog alone masks the complete path.
        let without = redact_with_username_row(variant, None);
        assert!(
            !without.contains(FIXTURE_USERNAME),
            "static catalog must catch non-Users paths (got {without:?})"
        );
    ...
    fn test_workspace_fixture_populates_identity_fields() {
        ...
        assert_eq!(ws.name, "Alice's ExampleGame");
