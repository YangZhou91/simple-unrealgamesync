...
//! - **Redacted at the persistence boundary** — paths and credentials use
//!   the same privacy filter as the main diagnostic log. Raw process output
//!   remains available to the parser and live UI, never to this file sink.
...
///
/// The raw p4 line is redacted before writing. `total` is printed verbatim;
...
pub fn format_sync_file_line(current: u64, total: u64, raw: &str) -> String {
    ...
    format!("{current}/{total} {}", super::redact::redact(trimmed))
}
...
mod tests {
    ...
    // ---- format_sync_file_line: privacy + count format ----

    #[test]
    fn format_line_redacts_raw_path() {
        // Retain the counter while removing private paths at the file boundary.
        let raw = r"C:\Users\alice\workspaces\ExampleGame\Foo.uasset";
        let out = format_sync_file_line(5, 100, raw);
        assert!(out.starts_with("5/100 "), "expected '5/100 ' prefix in {out:?}");
        assert_eq!(out, "5/100 <PATH>");
    }
    ...
    fn format_line_never_contains_trailing_newline() {
        ...
        let out = format_sync_file_line(1, 10, "//ExampleGame/Main/Content/Maps/Foo.uasset");
    ...
    fn format_line_total_zero_does_not_panic() {
        ...
        let out = format_sync_file_line(5, 0, r"C:\Users\alice\workspaces\ExampleGame\Foo.uasset");
        assert!(
            out.starts_with("5/0 "),
    ...
    fn format_line_preserves_non_path_content() {
        // A clean business line survives intact.
    ...
    #[test]
    fn write_line_appends_redacted_lines() {
        let dir = unique_temp_dir("write_one");
        let mut writer =
            SyncRunFileWriter::open(&dir, "run1").expect("open on real dir returns Some");
        writer.write_line(1, 4, r"C:\Users\alice\workspaces\ExampleGame\A.uasset");
        writer.write_line(2, 4, r"C:\Users\alice\workspaces\ExampleGame\B.uasset");
        writer.flush();
        drop(writer);
    ...
    fn flush_and_drop_do_not_panic() {
        ...
        writer.write_line(7, 9, r"C:\Users\alice\workspaces\ExampleGame\C.uasset");
        writer.flush();
        ...
        writer2.write_line(1, 1, r"C:\Users\alice\workspaces\ExampleGame\D.uasset");
        drop(writer2);
        cleanup(&dir);
    }
