...
/// string is routed through `redact()` again so depot paths (`//ExampleGame/...`),
/// tagged values, and any unexpected path are net-caught. The underlying
/// catalog masks arbitrary depot and Windows absolute paths.
...
mod tests {
    ...
    fn spawned_line_masks_client_and_root() {
        ...
        // `alice-laptop-examplegame` and the bare username `alice` MUST NOT appear
        ...
        let line = render_spawned_line(1234, &ws.root_path, "sync //ExampleGame/...@310771");
        assert!(
    ...
    fn file_formatter_redaction_masks_path_in_body() {
        ...
        let body = format_line_body(
            ...
            log::Level::Info,
            "——",
            "[sync] path=C:\\Users\\alice\\ExampleGame",
        );
